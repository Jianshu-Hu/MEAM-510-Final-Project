...
// task handle for use in the notifications
...
void ShowHealth(int health)
{
    ...
    int totalH = 5;
    int unit = ceil(float(health)/float(totalH)*20);
    int i = 0;
    for(i = 0;i < unit;i++)
    {
...
{
    // TODO: implement this function
    int unit = ceil(float(respawnTime)/15*24);
    int i = 0;
    for(i = 0;i < unit;i++)
    {
      leds[i] = RED;
...
//    gameStatus = 1;////////////////////////////////////////////////////////////////need to delete
    if(gameStatus == 0)
    {
      Serial.println("game doesn't start");
      servo1.detach();
      servo2.detach();
      servo3.detach();
      servo4.detach();
...
//        health = 2;//////////////////////////////////////////////////////need to delete
        
        if(health == 0 || switch1_signal == 0)
        {
          servo1.detach();
          servo2.detach();
          servo3.detach();
...
void UDPreceiveData(){
  ...
  if(cb)
  {
    ...
    int weapon_infor = receiveBuffer[4] | (receiveBuffer[5] << 8);
    if(motor_infor1 == 257)
    {
      motor_infor1 = 0;
    }
...
{
    //judge the range of control and set the speed of two speed
    int speed_left = 90;
    int speed_right = 90;
    int speed_initial = 90;
    int y = motor_speed;
    int x = motor_diff;

    //stay still
//    if( (x>1847) && (x<2247) && (y>1847) && (y<2247) )
//    {
//      servo1.detach();
//      servo2.detach();
//      servo3.detach();
...
//    else
    ...
    if( ( (y>=4095-x) && (y>=x) ) || ( (y<=4095-x) && (y<=x) ) )
    {
      speed_initial = map(motor_speed,0,4095,0,180);
      speed_left = speed_initial;
      speed_right = 180-speed_initial;
      if(speed_initial > 100)
      {
    ...
    if( ( (y>4095-x) && (y<x) ) || ( (y<4095-x) && (y>x) ) )
    {
      speed_initial = map(motor_diff,0,4095,0,180);
      speed_left = speed_initial;
      speed_right = speed_initial;
      if( (speed_left > 100) || (speed_left<80) )
      {
        digitalWrite(led_l,HIGH);
        digitalWrite(led_r,HIGH);
      }
...
//rotate the weapon
void weapon_rotate(int weapon_infor)
{
//   if( (weapon_infor>1847) && (weapon_infor<2247) )
//   {
//    servo5.detach();
//   }
//   else
   ...
   int weapon_speed = map(weapon_infor,0,4095,180,0);
   servo5.write(weapon_speed);
